    ...
    public class OffsetCurveSetBuilder
    {
        ...
        private void AddPolygon(IPolygon p)
        {
            ...
            for (var i = 0; i < p.NumInteriorRings; i++)
            {
                ...
                AddPolygonRing(holeCoord, offsetDistance, NetTopologySuite.GeometriesGraph.Position.Opposite(offsetSide),
            ...
            Positions side, Location cwLeftLoc, Location cwRightLoc)
        {
                ...
                && CGAlgorithms.IsCCW(coord))
            {
                leftLoc = cwRightLoc;
                rightLoc = cwLeftLoc;
                side = NetTopologySuite.GeometriesGraph.Position.Opposite(side);
            }
