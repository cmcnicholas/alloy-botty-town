...
namespace NetTopologySuite.Index.KdTree
{
    ...
    /// </remarks>
    /// <typeparam name="TOrig">The type of the user data object</typeparam>
    /// <author>David Skea</author>
    /// <author>Martin Davis</author>
    public class KdTree<TOrig>
        where TOrig : class
    {
        ///<summary>
        ...
        public static Coordinate[] ToCoordinates(ICollection<KdNode<TOrig>> kdnodes)
        {
        ...
        /// <returns>An array of the coordinates represented by the nodes</returns>
        public static Coordinate[] ToCoordinates(ICollection<KdNode<TOrig>>  kdnodes, bool includeRepeated)
        {
        ...
        public KdNode<TOrig> Insert(Coordinate p)
        {
        ...
        /// </returns>
        public KdNode<TOrig> Insert(Coordinate p, TOrig data)
        {
            if (_root == null)
            {
                _root = new KdNode<TOrig>(p, data);
                return _root;
            }
        ...
        /// </list>
        /// </returns>
        private KdNode<TOrig> FindBestMatchNode(Coordinate p)
        {
            var visitor = new BestMatchVisitor<TOrig>(p, _tolerance);
        ...
        /// </list>
        /// </returns>
        public KdNode<TOrig> InsertExact(Coordinate p, TOrig data)
        {
...
// ReSharper disable once PossibleNullReferenceException
            ...
            var node = new KdNode<TOrig>(p, data);
            node.Left = null;
            node.Right = null;
            if (isLessThan)
            {
        ...
        private static void QueryNode(KdNode<TOrig> currentNode,
            Envelope queryEnv, bool odd, IKdNodeVisitor<TOrig> visitor)
        {
            if (currentNode == null)
                return;
        ...
        public void Query(Envelope queryEnv, IKdNodeVisitor<TOrig> visitor)
        {
        ...
        public IList<KdNode<TOrig>> Query(Envelope queryEnv)
        {
            var result = new List<KdNode<TOrig>>();
            QueryNode(_root, queryEnv, true, new KdNodeVisitor<TOrig>(result));
            return result;
        }
        ...
        public void Query(Envelope queryEnv, IList<KdNode<TOrig>> result)
        {
            QueryNode(_root, queryEnv, true, new KdNodeVisitor<TOrig>(result));
        }

        private static void NearestNeighbor(KdNode<TOrig> currentNode,
            Coordinate queryCoordinate, ref KdNode<TOrig> closestNode, ref double closestDistanceSq)
        {
            while (true)
            {
                if (currentNode == null)
        ...
        public KdNode<TOrig> NearestNeighbor(Coordinate coord)
        {
            KdNode<TOrig> result = null;
